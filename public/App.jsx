...
async function hashPw(pw) {
  ...
  const bits = await crypto.subtle.deriveBits({ name:"PBKDF2", salt:enc.encode("labbook-salt-v1"), iterations:100000, hash:"SHA-256" }, key, 256);
...
function loadSession() {
  try {
    ...
    if (!s) return null;
    if (Date.now() - (s.lastActive || 0) > 24 * 60 * 60 * 1000) { localStorage.removeItem("lb_session"); return null; }
    return s;
  } catch { return null; }
}
function saveSession(s) { try { localStorage.setItem("lb_session", JSON.stringify(s ? {...s, lastActive: Date.now()} : null)); } catch {} }
function loadTheme() {
  try {
...
export default function App() {
  ...
  function navigate(v, data={}) { setView(v); setViewData(data); }
  ...
  async function login(username, password) {
    ...
    const oldHash = (function(pw){ let h=0; for(let i=0;i<pw.length;i++){h=((h<<5)-h)+pw.charCodeAt(i);h|=0;} return h.toString(16); })(password);
    if (acc.password_hash !== newHash && acc.password_hash !== oldHash) return "Incorrect password.";
    if (acc.password_hash === oldHash) {
      try { const db = await sb.from("accounts"); await db.update({ password_hash: newHash }, { id: acc.id }); setAccounts(p => p.map(a => a.id===acc.id ? {...a, password_hash: newHash} : a)); } catch(e) {}
    }
  ...
  async function signup(username, displayName, password) {
    ...
    try {
      ...
      setSession(s);
      saveSession(s);
      showToast(`Welcome, ${newAcc.display_name.split(' ')[0]}! 🎉`);
      return null;
    } catch (e) {
...
function ScheduleView({
  ...
  const [dragVis, setDragVis] = useState(null); // { startDate, startMins, endDate, endMins } — for rendering only
    ...
    <div className="view">
      ...
      >
        <button
          ...
          style={{
            ...S.filterPill,
            background: filterInst === 'all' ? '#38BDF8' : t.bg1,
            color: filterInst === 'all' ? '#000' : t.textLow,
            border: `1px solid ${filterInst === 'all' ? '#38BDF8' : t.border}`,
          }}
        >
          All
        </button>
        {instruments.map((i) => (
          <button
            ...
            style={{
              ...S.filterPill,
              background: filterInst === i.id ? i.color : t.bg1,
              color: filterInst === i.id ? '#000' : t.textLow,
              border: `1px solid ${filterInst === i.id ? i.color : t.border}`,
            }}
          >
            {i.icon} {i.code}
          </button>
            ...
            {instruments.map((i) => (
              <button
                ...
                style={{
                  ...
                  fontSize: 11,
                  background:
                    instrumentId === i.id ? i.color + '33' : '#0B1628',
                  color: instrumentId === i.id ? i.color : '#64748B',
                  border: `1px solid ${
                    instrumentId === i.id ? i.color : '#1E3A5F'
                  }`,
                }}
              >
                {i.icon} {i.code}
    ...
    <div className="view">
      ...
      {past.length > 0 && (
        <>
          ...
          {past.map((b) => (
            <BookingCard key={b.id} b={b} instruments={instruments} past />
          ))}
        </>
      )}
    </div>
...
function BookingCard({ b, instruments, onCancel, past }) {
  const inst = instruments.find((i) => i.id === b.instrument_id);
  if (!inst) return null;
  const dur = durationMins(b.start_time, b.end_time);
  return (
...
function AdminView({
  ...
  return (
    <div className="view">
      ...
      >
        {['instruments', 'bookings', 'users', 'stats'].map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            style={{
      ...
      {tab === 'users' && (
        <>
          ...
          >
            {accounts.length} MEMBER{accounts.length !== 1 ? 'S' : ''}
          </div>
          {accounts.map((a) => (
            <div
              key={a.id}
