  ...
  function TimeColumn({ date, bkgs }) {
    ...
    return (
      ...
      >
        ...
        {dragOverlay}
        {(() => {
          // Each instrument gets a fixed column based on its position in the
          // instruments array — so the same instrument always appears in the
          // same vertical lane regardless of what else is booked that day.
          const instIds = instruments.map((i) => i.id);
          // Only count instruments that actually have bookings on this day
          const activeInstIds = instIds.filter((id) =>
            bkgs.some((b) => b.instrument_id === id)
          );
          const colTotal = activeInstIds.length || 1;
          return bkgs.map((b) => {
            const colIndex = activeInstIds.indexOf(b.instrument_id);
            return (
              <BookingBlock
                key={b.id}
                b={b}
